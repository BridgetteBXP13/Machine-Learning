...
#### Training
Here we will create our AdaBag model.
```{r}
library(adabag)
set.seed(1010)
i <- sample(1:nrow(full_stats_train), .001*nrow(full_stats_train), replace=FALSE)
smaller_train <- full_stats_train[i,]
adab_model <- boosting(won~., data=smaller_train, boos=TRUE, mfinal=3, coeflearn='Breiman')
summary(adab_model)
```

#### Testing & Evaluation
Here we will predict with and evaluate the results our AdaBag model
```{r}
pred_adab <- predict(adab_model, newdata=full_stats_test, type="response")
pred_adab$class
```


## Random Forest vs XGBoost vs AdaBag
