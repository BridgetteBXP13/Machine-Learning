...
```{r}
set.seed(1010)
j <- sample(1:nrow(full_stats_df), 0.3*nrow(full_stats_df), replace=FALSE)
full_stats_df <- full_stats_df[j,]
spec <-c(train=.6, test=.2, validate=.2)
i <- sample(cut(1:nrow(full_stats_df), nrow(full_stats_df)*cumsum(c(0,spec)), labels=names(spec)))
full_stats_train <- full_stats_df[i=="train",]
full_stats_test <- full_stats_df[i=="test",]
full_stats_validate <- full_stats_df[i=="validate",]
str(full_stats_train)
```
...
confusionMatrix(as.factor(svm_probs), reference = as.factor(full_stats_test$won))
```
## kNN Classification
