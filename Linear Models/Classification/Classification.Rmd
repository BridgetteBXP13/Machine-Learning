...
fun <- function(dataf, i){
  ...
  print(confusionMatrix(data=as.factor(pred), reference=funtest$Cover_Type))
...
prbayes <- prediction(as.numeric(pbayes), as.numeric(test$Cover_Type))
prfbayes <- performance(prbayes, measure="tpr", x.measure="fpr")
plot(prfbayes)
aucbayes <- performance(prbayes, measure = "auc")
print(paste("AUC: ", aucbayes))
```
...
Paragraph discussing the strengths and weaknesses of Naive Bayes and Logistic Regression
