...
Type 5:
For this one it only shows us the truth line. We aren't sure what caused this bug or why it is only doing it for tree type 5, but regardless it is saying that it is still accurately predicted true positive at a 98% accuracy.
...
Type 7:
This model had around a 98% accuracy level. As with the other models its reporting a lot of true positives, as well as false positives. This one however has more true negatives than 
```{r}
tree_type1 <- df
